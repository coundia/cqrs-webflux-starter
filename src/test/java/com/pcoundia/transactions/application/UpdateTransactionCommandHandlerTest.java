...
@SpringBootTest
public class UpdateTransactionCommandHandlerTest extends BaseUnitTests {
...
void setUp() {
	...
	new TransactionId(UUID.randomUUID().toString()), new TransactionReference(UUID.randomUUID().toString()), new TransactionAmount(1941.56)
	);
}

@Test
void it_should_handle_command() {
	when(repository.save(any())).thenReturn(Mono.just(new Transaction()));
