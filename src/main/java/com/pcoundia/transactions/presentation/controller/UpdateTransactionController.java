...
public Mono<ApiResponseDto> updateTransaction(
	...
	UpdateTransactionCommand command = TransactionMapper.toUpdateCommand(TransactionId.create(id), transactionRequest);
	...
	return Mono.just(ApiResponseDto.error("Failed to update transaction: " + ex.getMessage()));
	});
	}
}
